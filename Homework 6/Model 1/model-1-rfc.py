...
def fetch_data():
    with pool.connect() as db_conn:
        result = db_conn.execute(sqlalchemy.text("SELECT client_ip, country, requested_file FROM Requests WHERE requested_file LIKE '%%.html'"))
...
df = pd.get_dummies(df, columns=['client_ip'], drop_first=True)
...
print(f"Model 1 - Random Forest Classifier : Test Accuracy = {accuracy * 100:.2f}%")
